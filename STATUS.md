...
## Most recent — live-charge rehearsal caught the separate-charges fee bug; fix implemented (Sep 15, 2026, UNCOMMITTED — pending Chris review)

The first live Stripe charge attempt (`POST /api/admin/mark-delivered`, line item `9701e646-2ad5-45c0-98b2-9a43fce40809`) failed 502: `chargeForEpisode` passed `application_fee_amount` on a platform-account PaymentIntent — Stripe only accepts that on direct/destination charges, and Taylslate is **separate charges & transfers** (platform charge → show-net transfer). The failed run also left the line item `verified=true` with no payment. Fix implemented on the working tree, **NOT committed / NOT deployed** (Chris reviews first). **1192 tests (107 files)** green, tsc clean. **No migration** (`domain_events.event_type`/`entity_type` are plain TEXT).

- **Fee removed from the PI** (`lib/stripe/payment-intent.ts`). Still computed + snapshotted to `payments.application_fee_amount_cents` (the transfer's source of truth) and now carried on PI metadata for dashboard reconciliation only. Verified repo-wide: no code reads the fee back from Stripe objects; webhooks touch only `status`/`settled_at`.
- **Idempotency key versioned** → `pi:v2:{dealId}:{ioLineItemId}`. The v1 key is bound at Stripe (~24h) to the rejected params; a same-key retry with fixed params would return `idempotency_error`. Standing rule (comment at the create call): any change to the create-params shape bumps the version. Transfer keys (`transfer:`/`transfer-early:{paymentId}`) audited — safe, params derive from immutable payments-row snapshots.
- **`chargeForEpisode` is now DB-idempotent on (deal, line item)**: a payments pre-check short-circuits on an existing non-failed Stripe-backed row without touching Stripe (fails closed on read error), and a 23505 on the insert resolves to the existing row. Retries converge to success instead of tripping the unique index.
- **mark-delivered rolls back on charge failure** to the exact pre-request snapshot (prior `verified`/`actual_post_date`/`actual_downloads`, not blanket false/null) and returns `ok: false` + `rolledBack` (+ `rollbackError` if the restore itself failed). New domain events `io_line_item.delivered` / `io_line_item.delivery_rolled_back` (+ `io_line_item` entity type in `lib/data/types.ts`). Charge-succeeds-then-insert-fails heals on retry: v2 key replays the same PI (no double charge), insert gets a second chance.
- **Ops sequence for the re-rehearsal (after review + deploy):** (1) reset the dirty line item — `update public.io_line_items set verified = false, actual_post_date = null where id = '9701e646-2ad5-45c0-98b2-9a43fce40809';` — so the re-run exercises the real verify-then-charge path; (2) re-POST mark-delivered → expect 200, `payments` row with `application_fee_amount_cents`, PI at Stripe with **no** application fee but fee metadata; (3) confirm `charge.succeeded` → `settled_at` → transfer of show net (Connect fixture `acct_1UENUbCNreR40F4c`). The failed v1 attempt created no PI at Stripe — nothing to clean up there.
