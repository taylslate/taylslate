...
- **contact email — NOT a gap.** Live-verified **97.7% fill** across 86 discovered shows (95.8% on the 5k–25k tier, 100% at 25k+; 3 mainstream-vertical briefs, zero Podscan errors). `contact.email` is `reach.email ?? ""`; the outreach "to" address at send time is `outreach.sent_to_email`, seeded from this. Whether an address is a real booking contact vs. a generic RSS/owner address is answered by real bounce/reply rates, not more sampling.
- **`shows.demographics` — always `{}` in prod (biggest real gap).** The live conviction path (`runConvictionDiscovery`) never fetches demographics; every discovered show is hardcoded `demographics: {}` (`lib/discovery/format-discovered-show.ts`). The Podscan demographics fetch (`getPodcastDemographics`) + the audience-fit scorer that would consume it live only in the **Wave 5 scorer (`lib/scoring/index.ts` + `scoring/dimensions/audience-fit.ts`), which is dead code** — nothing imports it — so its neutral-fallback path is orphaned and never executes.
- **`shows.rss_url` — store-only string.** Written from Podscan at discovery/enrich; the feed is **never fetched or parsed** anywhere.
...
## Fixture — Stripe Connect payout test account (recorded Sep 11, 2026)

**`acct_1UENUbCNreR40F4c` is the fixture Stripe Connect account for payout testing** — created manually via the Stripe dashboard (NOT through the product's Connect onboarding route), verified, CHASE bank account attached, contact email `chris+test1@taylslate.com`. Attached to `profiles.stripe_connect_account_id` for `chris+show1@taylslate.com` (`befd31bb-b5ac-4bb7-b2b5-b01f345c4363`). **First payout destination in the system — all other show profiles have NULL.** Note: the Connect contact email and the Taylslate login email intentionally differ — nothing keys off the match, and in production they routinely won't. Supports the settlement frontier (live Stripe charge / payout — see Next).

## Most recent — show countersign CTA on brand_signed deals (Sep 16, 2026)

Show users on a `brand_signed` deal now get a **Sign IO** button in the Signature status card. Clicking it reuses `POST /api/deals/[id]/send-to-docusign`, which mints a publisher recipient-view URL via `getShowSigningUrl` (routing-order 2, `clientUserId: "show"`). Brand viewers on the same deal do not see that CTA; the button is gone once status is `show_signed`. Envelope already exists — no stored sign URL, no deal writes from this path. Connect webhook remains the only writer for `show_signed_at` / PDFs. Closes the gap on deal `4bea8769` where the show had no in-app way to countersign.

## Most recent — SETTLEMENT FRONTIER: first live charge PROVEN; payout transfer fix implemented (Sep 15, 2026 — payout fix UNCOMMITTED, pending Chris review)

**The first live Stripe charge in the system's history ran end to end and settled** — but the payout transfer behind it failed, and its root fix is on the working tree awaiting review. The day in three acts:

- **Act 1 — separate-charges fee bug, SHIPPED (`ef35da0`, Vercel green).** The first live charge attempt failed 502: `chargeForEpisode` passed `application_fee_amount` on a platform-account PI — illegal outside direct/destination charges; Taylslate is separate charges & transfers. Fix: fee removed from the PI (still computed + snapshotted to `payments.application_fee_amount_cents`, the transfer's source of truth; carried on PI metadata for dashboard reconciliation only — verified nothing reads it back from Stripe); PI idempotency key versioned `pi:v2:{dealId}:{ioLineItemId}` (v1 poisoned at Stripe ~24h); `chargeForEpisode` made DB-idempotent on (deal, line item) via payments pre-check + 23505 tolerance; mark-delivered now rolls back the verify write to the exact pre-request snapshot on charge failure and returns `ok:false` (new domain events `io_line_item.delivered`/`.delivery_rolled_back`, entity `io_line_item`). No migration.
- **Act 2 — LIVE CHARGE PROVEN (22:08 UTC).** After the line-item reset (`9701e646…` → unverified), mark-delivered ran the real verify-then-charge path: HTTP 200, payment `6de1a669-f3fa-49bd-bbcb-f2816dde058b`, PI `pi_3UG4WcFwAaCc9OhA1gIKSo74`, $1.00 charged on a live Amex, fee snapshot 10¢ @ 10%, `settled_at` stamped by the `charge.succeeded` webhook ~1s later. Charge + settle rails are now live-proven.
- **Act 3 — payout transfer failed `balance_insufficient`; root fix implemented, UNCOMMITTED.** The webhook DID call `transferPayoutForPayment` (fail-soft caught it; Vercel log 22:08:28): `stripe.transfers.create` omitted `source_transaction`, so the transfer drew on the platform's *available* balance ($0.00 — card charges pend ~2 business days) instead of the funding charge. No Transfer object created, no payouts row, payout stranded (the webhook is the only caller and it acked 200, so Stripe won't redeliver). Fix on the working tree (`lib/payouts/transfer.ts`): **`source_transaction`** resolved at transfer time via `stripe.paymentIntents.retrieve(payments.stripe_payment_intent_id).latest_charge` — no migration, no backfill problem, works for the stranded payment; the settle gate guarantees the charge exists, and missing PI-id / `latest_charge` fail loud — on BOTH transfer paths (auto + early). Keys versioned **`transfer:v2:` / `transfer-early:v2:`** (param shape changed; `transfer:6de1a669…` has the insufficient-funds error cached until ~22:08 Sep 16). Zero-float is now structural: the show's money always rides the brand's charge and settles with it.
- **Recovery for the stranded 90¢ payout (after review + deploy):** Stripe dashboard → resend the `charge.succeeded` event for `pi_3UG4WcFwAaCc9OhA1gIKSo74` → webhook re-runs the real path with v2 key + `source_transaction` → expect payouts row (90¢ → fixture `acct_1UENUbCNreR40F4c`), `payout.transferred` event, transfer carrying `source_transaction`. Doubles as the live proof of the fix. (`settled_at` re-stamps; the original 22:08:29 is preserved in the `payment.settled` domain event.)
- **Backlogged (PRODUCT_BACKLOG.md → Foundational Architecture):** duplicate `payment.charged` writers (chargeForEpisode + the `payment_intent.succeeded` handler — analytics-only noise); durable retry for the stranded-payout failure class (transfer throws → webhook acks 200 → nothing retries).
- **Local env fact (recorded):** `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET` are EMPTY in `.env.local` — live Stripe keys exist only in Vercel env; local scripts cannot fire or query live Stripe.
...
**Pile A is COMPLETE (A1–A6).** The two seams left open after the Sep 3 G6 rail proof — A5 (email deliverability rehearsal) and A6 (outreach From:-name credibility) — were both verified live the same day. All six Pile A items are now live-proven. Recording-only pass (no code changed).

- **A5 — email deliverability VERIFIED.** A real outreach email delivered to an **external Gmail inbox with SPF, DKIM, and DMARC all passing** (authentication confirmed in the Gmail "show original" headers — not just soft inbox placement). Closes the standing "outreach has never been sent to a real external inbox; DMARC unconfirmed" gap; the rehearsal now produces valid deliverability signal.
- **A6 — outreach From:-name VERIFIED LIVE.** The durable `brand_name` field (migration 034, commit `2e7809d`) drives the outreach From: name, IO advertiser name, and pitch/notification sender through the shared helper — verified live rendering a real brand name, not the `brand_identity` paragraph. A6 was pulled into Pile A because a deliverability rehearsal with a broken From: line produces invalid signal; with the fix live, A5's signal is trustworthy.
- **Follow-up found during A6 verify (logged, not blocking):** the **public pitch page headline** still renders the `brand_identity` paragraph where the brand *name* belongs — same root cause as A6, one site missed in the reroute. Logged in PRODUCT_BACKLOG.md → Polish (prospect-facing credibility fix). The email path A6 covers is correct; this is a separate surface.
- **Next frontier — a live Stripe *charge* / settlement.** Card-on-file is proven end to end (A1–A4); a live charge / payout settlement has still never run. That is the settlement proof, distinct from (and after) Pile A.
...
**First live G6 proof.** The full A1–A4 chain ran end to end against **production DocuSign (`na4`) + live Stripe (`acct_1Kejm5Fw`)** — real envelope, real Connect delivery, real Amex. This closed the DocuSign→Stripe rail: **A1–A4 are now LIVE-PROVEN** (A5/A6 closed separately the same day — see the Pile A COMPLETE block above). Recording-only pass (no code changed); full ids + timeline in `PILE_A_PROOF.md` → "Live run — 2026-09-03".
...
- **A5/A6:** not exercised by *this* run — verified separately the same day (see the Pile A COMPLETE block above). No live charge was run in this pile (it proves card-on-file / ready-to-charge, not settlement). Accepted deviations unchanged: G1 (state-guard idempotency) + G5 (column-based card-on-file).
...
Four issues surfaced during the July 16 live-verify, out of the accept cluster's scope, now fixed. Migration **032** required (cadence CHECK widen) — applied + introspected before the dependent code. **1021 tests (94 files)**, tsc/eslint/`next build` clean. Commits `1e2d97a` (#1-3) + `9430bd8` (#4) + `875c936` (Codex-resolution). **Codex-reviewed** (all four fixes): no High; 3 Medium + 2 Low — resolved 3 Medium + 1 Low, 1 Low accepted (see below).

- **LIVE-VERIFIED July 23** (all four, launch-done): pitch-page flight dates correct (**Aug 6 – Sep 3**); the **"A few times a week"** cadence option is present and flows into IO post-date spacing (**Aug 6 / 9 / 12** = 3-day spacing); onboarding now returns **straight to the pitch with Accept immediately actionable** — no manual nav, no refresh; the **IO PDF** flight + post dates match the pitch/deal/email. Teardown cascade clean incl. the materialized show. Two small deferred copy items logged in PRODUCT_BACKLOG.md → Polish (onboarding summary button label; post-accept dead-end "Go to my dashboard" link).
...
against production `na4` + live Stripe `acct_1Kejm5Fw` (A1–A6 all live-proven —
**Pile A COMPLETE**). DocuSign go-live + production cutover are complete and now
exercised through a real Connect webhook. See the top "Most recent" block for ids +
timeline. **A5 (email deliverability — SPF/DKIM/DMARC to an external Gmail inbox)
and A6 (outreach From:-name) were verified live Sep 3, completing Pile A. The next
frontier is a live Stripe charge / settlement (card-on-file is proven; a charge has
never run).**
...
`PILE_A_PROOF.md`), and **A5 (email deliverability — SPF/DKIM/DMARC passing to an
external Gmail inbox) + A6 (outreach From:-name) verified live the same day**.
DocuSign go-live + production cutover are complete. **Next frontier: a live Stripe
*charge* / settlement (not just card-on-file — a charge has never run).** The
