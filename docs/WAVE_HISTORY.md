# Taylslate — Wave History & Archived Reference

Historical reference. Current state lives in STATUS.md; queue lives in PRODUCT_BACKLOG.md.

Everything below was extracted verbatim from CLAUDE.md during the July 6, 2026 restructure (CLAUDE.md was 615 lines and had accumulated wave-by-wave narrative and as-of-a-date reference). Nothing here is deleted knowledge — it is moved. For session-invariant rules, conventions, and gotchas, see CLAUDE.md. For strategy / competitive / discovery-thesis depth, see TAYLSLATE_CONTEXT.md.
...
> Note: the "501 tests / migrations 001–024" line that used to sit here is superseded — current test count and migration state live in STATUS.md. Wave 14 Phase 2 (2A–2D) shipped after this narrative was written; 2D COMPLETE July 6, 2026.
...
## Wave 14 — Discovery Agent (Phase 1 shipped, Phase 2 detail)
...
### Wave 14 Phase 2 — Discovery Agent UX (shipped; 2D COMPLETE July 6, 2026)

Wires Phase 1 dormant infrastructure into the brand-facing UI. Sub-phases (each was its own Claude Code session):

1. **2A — Brief intake redesign + interpretation loop (~3 days).** Reshape current 9-step form into free-text-led intake (product / customer / campaign sections). AI proposes 1 primary read + 2-4 lateral candidate ring hypotheses with confidence. Brand confirms or refines interpretation. Wires `recordCampaignPattern()` and `recordRingHypothesis()`. Layer 4 (brief interpretation) hardened across two Codex amendments — migration 022 (`analog_matches.analog_pattern_id` FK + `interpretation_locks` sentinel) and migration 024 (`persist_interpretation` — pattern + rings + analogs in one transaction; the `campaign_patterns` row's atomic visibility is the completion sentinel, and crash-orphan locks expire by TTL in `lib/data/interpretation-lock.ts`).
...
3. **2C — Test portfolio + scale tier dual output (~3 days).** Discovery returns two distinct lists from one analysis. Test portfolio with 3-spot floor as default budget filter. Scale tier surfaces high-conviction shows that exceed test budget with "deferred — fits future budget" framing. (Migration 028 conviction cost/curation; migration 029 Layer 5 override inputs. Layer 5 overrides UI remains optional polish.)

4. **2D — Founder annotations + show brand history + promo code capture (~2 days).** Founder annotation UI for show-level reasoning (L1). Show onboarding "brand history" field (L2). Promo code field at IO generation time (Layer A, migration 030 `deals.promo_code`). Auto-generated UTM-tagged tracking link per deal (Layer B, generated on read). Show notes blurb generation helper (Layer C, deterministic template, generated on read).
...
supabase/migrations/                # SQL migrations (see STATUS.md for applied state)
docs/
...
- **SpotsNow (added June 1, 2026)** — launched an AI podcast campaign planner on top of its remnant/open-inventory marketplace. Generic "enter URL and budget, get shows" is no longer differentiated; Wave 14 Phase 2 is the anti-SpotsNow wedge.

See `TAYLSLATE_CONTEXT.md` for full competitive analysis.

## Build Queue (Wave-Sequenced) — snapshot at restructure

Pre-launch backlog and post-launch customer-driven items live in `PRODUCT_BACKLOG.md` (source of truth). Waves remain the execution unit. This is the queue as it stood at the July 6, 2026 restructure.
...
"Log me in as test show 1" admin button to replace Gmail +tag aliases. (Impersonation tool Layers 1+2 shipped June 28, 2026; test-deal seeding tool is next per PRODUCT_BACKLOG.md.)
