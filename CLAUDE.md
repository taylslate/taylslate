...
- **The "Confirm signup" email template uses the token_hash pattern** (manual dashboard config, set July 8, 2026): `{{ .SiteURL }}/callback?token_hash={{ .TokenHash }}&type=signup&next=/onboarding` — NOT `{{ .ConfirmationURL }}`. This makes confirmation server-verifiable via `verifyOtp` and browser-independent (a link opened on a different device than signup works); reverting to `{{ .ConfirmationURL }}` silently reintroduces PKCE cross-device fragility. `/callback` accepts `type=signup` via `OTP_TYPES`. Same token_hash rationale as the admin `generateLink` path above.
