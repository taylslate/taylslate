...
- **The "Confirm signup" and "Reset Password" email templates use the token_hash pattern** (manual dashboard config, set July 8, 2026): `{{ .SiteURL }}/callback?token_hash={{ .TokenHash }}&type=signup&next=/onboarding` and `…&type=recovery&next=/reset-password` respectively — NOT `{{ .ConfirmationURL }}`. This makes confirmation/recovery server-verifiable via `verifyOtp` and browser-independent (a link opened on a different device than the request works); reverting to `{{ .ConfirmationURL }}` silently reintroduces PKCE cross-device fragility. `/callback` accepts both `signup` and `recovery` via `OTP_TYPES`; on `recovery` it sets the short-lived HttpOnly `tslate_pw_recovery` marker that gates the `/reset-password` form (a passwordless magic-link show must never reach it). Same token_hash rationale as the admin `generateLink` path above.
...
- **Turbopack + Stripe/DocuSign SDKs (UMD) need THREE things together** (`lib/stripe/server.ts`, `lib/docusign/client.ts`, `next.config.ts`); removing any one breaks prod. (1) Load via `createRequire(import.meta.url)` inside a `new Function("require", …)` body — NOT `(0,eval)("require")`, which threw `require is not defined` at runtime because Turbopack emits these as ESM server modules with no global `require`. (2) The `new Function` keeps the specifier opaque so the bundler can't trace the AMD/UMD wrapper (`TP1200` build failure). (3) BUT that opacity also hides the packages from Next's file tracer, so `serverExternalPackages` alone does NOT pack them → runtime `Cannot find module 'docusign-esign'`. `next.config.ts` force-includes each SDK's **full transitive closure** (computed from `node_modules` at build time) via `outputFileTracingIncludes["/api/**"]`. Verify after changes by grepping the route's `.next/server/**/route.js.nft.json` for `node_modules/docusign-esign/`. Do NOT revert to eval-require or drop the tracing includes.
